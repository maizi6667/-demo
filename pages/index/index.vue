  ...
  <view class="container">
    ...
    <view class="banner">
      <view class="banner-left">
        ...
        <button class="publish-btn" @click="publishTrip">
          发布行程
        </button>
      </view>
   ...
   <view class="section">
    ...
    <view class="section">
      ...
      >
        ...
        <view class="trip-info">
          <view class="user-row">
            <text class="username">{{ trip.userName || trip.username }}</text>
            <text class="tag">{{ trip.tag || '实名认证' }}</text>
          </view>

          <view class="route">
            {{ trip.startPlace || trip.start_place }} → {{ trip.endPlace || trip.end_place }}
          </view>

          <view class="time">
            {{ trip.departTimeDisplay || trip.time }}
          </view>
        </view>

        <view class="trip-right">
          <view class="seat">{{ trip.seatCount || trip.seats }}个座位</view>
          <button 
            class="join-btn" 
      ...
      <!-- 空状态 -->
      <view v-if="!loading && latestTrips.length === 0" class="empty-text">
        暂无行程数据
      </view>
    </view>
    ...
    <view class="tabbar">
      ...
      <view class="publish-center" @click="publishTrip">
        ➕
      </view>
...
export default {
  ...
  methods: {
	  ...
	  viewMoreRoutes() {
	     uni.navigateTo({
	       url: '/pages/route-hot-list/route-hot-list'
	     })
	   },
	   
	   // 根据路线搜索
	   searchByRoute(route) {
	     // 解析路线名称，格式如："发送的 → FAFDA"
    ...
    openApply(tripId, nickname, route) {
       console.log('openApply收到tripId:', tripId)  // ← 加这行
...
console.log('this{}',this.submitting)
      request({
...
goMassage(){
    ...
    getHotRoutes() {
      request({
        url: '/carpool/routes/hot',
        method: 'GET'
      }).then(res => {
        console.log('热门路线接口返回:', res)
    ...
    // 获取最新行程（调用 /carpool/routes/latest 接口）
    getLatestTrips() {
      this.loading = true
      request({
        url: '/carpool/routes/latest',
        method: 'GET'
      }).then(res => {
          console.log('接口原始第一条:', JSON.stringify(res.data[0]))  // ← 加这行
  
        console.log('最新行程接口返回:', res)
        if (res.code === 0 && res.data && Array.isArray(res.data)) {
         ...
         // getLatestTrips 里，不要搞双份
return {
    tripId: item.tripId || item.id,  // 兜底两种可能
  userName: item.userName,  // 只用 userName
...
.banner {
  margin: 20rpx;
  background: linear-gradient(135deg, #c8f7dc, #e6fff1);
  border-radius: 30rpx;
  padding: 30rpx;
  display: flex;
  align-items: center;
